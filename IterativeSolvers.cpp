...
int IterativeSolvers::successiveOverRelaxing(const double omega, const matrix& A, matrix& x, const matrix& b)
{
	if (!(A.numcols() == A.numrows()) || !(A.numcols() == b.numrows()))
	{
		cerr << "A nRows: " << A.numrows() << endl;
...
//			std::cerr << " " << i;
			...
			x_new[i][0] = x_old_i + (omega * ((bMinusSigma / A[i][i]) - x_old_i));
